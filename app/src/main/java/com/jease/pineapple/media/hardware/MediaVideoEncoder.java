...
public class MediaVideoEncoder extends MediaEncoder {
     ...
     * 高清录制码率倍数
     */
    private static final int HDValue = 2;
    ...
    private void setupHDParams(MediaFormat format) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            for (int i = 0; i < numCodecs; i++) {
                ...
                for (int k = 0; k < mimeTypes.length; k++) {
                    if (mimeTypes[k].equalsIgnoreCase(MIME_TYPE)) {
    ...
    private boolean setProfileLevel(MediaFormat format, MediaCodecInfo codecInfo) {
        ...
        boolean isSupportedHighProfile = false;
        boolean isSupportedMainProfile = false;
        int profile = MediaCodecInfo.CodecProfileLevel.AVCProfileBaseline;
        int level = 1;
        ArrayList<Integer> highLevels = new ArrayList<>();
        ArrayList<Integer> mainLevels = new ArrayList<>();
        for (int i = 0; i < profileLevels.length; i++) {
            MediaCodecInfo.CodecCapabilities c = MediaCodecInfo.CodecCapabilities.createFromProfileLevel(MIME_TYPE,
                    profileLevels[i].profile, profileLevels[i].level);
            if (c == null) continue;
            for (int j = 0; j < c.profileLevels.length; j++) {
        ...
        if (isSupportedHighProfile) {
            ...
            Collections.sort(highLevels);
            level = highLevels.get(highLevels.size() - 1);
            result = true;
        } else if (isSupportedMainProfile) {
            profile = MediaCodecInfo.CodecProfileLevel.AVCProfileMain;
            Collections.sort(mainLevels);
            level = mainLevels.get(mainLevels.size() - 1);
            result = true;
        }
...
//        	MediaCodecInfo.CodecCapabilities.COLOR_QCOM_FormatYUV420SemiPlanar,
    ...
    private void enableHighDefinition(boolean enable) {
        isEnableHD = enable;
    }
}
